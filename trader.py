...
def get_holdings_and_bought_price(stocks):
    ...
    for stock in stocks:
        try:
            ...
            bought_price[stock] = float(
                (rh_holdings[stock]['average_buy_price']))
        except:
            holdings[stock] = 0
...
def display_ownings(stocks):
    ...
    for stock, qty in holdings.items():
        if qty > 0:
            print(
                f"Stock: {stock} | Quantity: {qty} | Average Buy Price: ${bought_price[stock]}")
...
def display_stock_details():
    ...
    for stock, details in holdings.items():
        ...
        print(f"Equity Value: ${details['equity']}")
        print(f"Quantity Owned: {details['quantity']}")
        print(f"Percent Change: {details['percent_change']}%")
        print(f"Equity Change: ${details['equity_change']}")

        # Calculate and display profit/loss
        average_buy_price = float(details['average_buy_price'])
        current_price = float(details['price'])
        quantity_owned = float(details['quantity'])
        profit_or_loss = (current_price - average_buy_price) * quantity_owned
...
if __name__ == "__main__":
    ...
    #     df_trades, df_prices = build_dataframes(
    #         df_trades, trade_dict, df_prices, price_dict)
