...
class trader():
    ...
    def trade_option(self, stock, price):
        ...
        if self.run_time % (5) == 0:
            df_historical_prices = self.get_historical_prices(
                stock, span='day')
            self.sma_hour[stock] = self.get_sma(
                stock, df_historical_prices[-12:], window=12)

        self.price_sma_hour[stock] = self.get_price_sma(
            price, self.sma_hour[stock])
        p_sma = self.price_sma_hour[stock]

        i1 = "BUY" if self.price_sma_hour[stock] < (
            1.0-self.buffer) else "SELL" if self.price_sma_hour[stock] > (1.0+self.buffer) else "NONE"
        if i1 == "BUY":
            trade = "BUY"
        elif i1 == "SELL":
            trade = "SELL"
